...
exports.getVendorStatus = async (req, res) => {
  try {
    ...
    // Find all submissions for this vendor in the specified period
    const submissions = await DocumentSubmission.find({
      vendor: vendorId,
      'uploadPeriod.year': parseInt(year),
      'uploadPeriod.month': month
    });

    // If no submissions found, return empty data
...
exports.getSubmissionDetails = async (req, res) => {
  try {
    ...
    if (
      (req.user.role === 'vendor' && submission.vendor._id.toString() !== req.user.id) ||
      (req.user.role === 'consultant' && submission.consultant && submission.consultant._id.toString() !== req.user.id)
    ) {
      return res.status(403).json({
        success: false,
