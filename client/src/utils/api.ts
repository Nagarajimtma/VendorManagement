...
const apiService = {
  ...
  documents: {
    ...
    getVendorStatus: (vendorId: string, year: number, month: string) =>
      api.get(`${API_PREFIX}/document-submissions/vendor-status`, { 
    ...
    downloadFile: (filePath: string, fileName?: string) =>
      api.get(`${API_PREFIX}/document-submissions/download`, {
        params: { filePath, fileName, format: 'pdf' }, // Include fileName parameter
        responseType: 'blob',
        headers: {
    ...
    viewFile: (filePath: string) => {
      ...
      // If the path starts with /uploads/, remove it as the server will add it back
      if (processedPath.startsWith('/uploads/')) {
        processedPath = processedPath.substring('/uploads/'.length);
