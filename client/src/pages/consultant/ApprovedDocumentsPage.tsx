                  ...
                  <div className="flex flex-col md:flex-row md:items-center md:justify-between">
                    ...
                    <div className="mt-2 md:mt-0 flex items-center space-x-2">
                      ...
                      <Button
                        variant="outline"
                        size="sm"
                        onClick={() => toggleDocumentDetails(document._id)}
                      >
                        {expandedDocument === document._id ? 'Hide Details' : 'View Details'}
                      </Button>
                      <Button
                        size="sm"
                        as={Link}
                  ...
                  {expandedDocument === document._id && (
                    <div className="mt-4 border-t border-gray-200 dark:border-gray-700 pt-4">
                      <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                        <div>
                          <h4 className="text-sm font-medium text-gray-500 dark:text-gray-400 mb-2">Document Details</h4>
                          <div className="space-y-2">
                            <div className="flex items-center">
                              <CalendarIcon className="h-5 w-5 text-gray-400 mr-2" />
                              <span className="text-gray-700 dark:text-gray-300">
                                Submitted: {document.submissionDate ? new Date(document.submissionDate).toLocaleDateString() : 'Unknown Date'}
                              </span>
                            </div>
                            <div className="flex items-center">
                              <UserIcon className="h-5 w-5 text-gray-400 mr-2" />
                              <span className="text-gray-700 dark:text-gray-300">
                                Reviewed by: {document.reviewer?.name || 'Unknown Reviewer'}
                              </span>
                            </div>
                            {document.reviewNotes && (
                              <div className="flex items-start">
                                <DocumentTextIcon className="h-5 w-5 text-gray-400 mr-2 mt-0.5" />
                                <span className="text-gray-700 dark:text-gray-300">{document.reviewNotes}</span>
                              </div>
                            )}
                          </div>
                        </div>
                        <div>
                          <h4 className="text-sm font-medium text-gray-500 dark:text-gray-400 mb-2">Files</h4>
                          <div className="space-y-2">
                            {document.files && document.files.length > 0 ? (
                              document.files.map(file => (
                                <div key={file._id} className="flex items-center justify-between bg-gray-50 dark:bg-gray-800 p-2 rounded">
                                  ...
                                  <Button
                                    ...
                                    onClick={() => downloadFile(file.path, file.originalName || 'document.pdf')}
                                  >
                                    Download
                                  </Button>
                                </div>
                              ...
                              <div className="text-gray-500 dark:text-gray-400 italic">No files available</div>
                            )}
                          </div>
                        </div>
                      </div>
                      ...
                      <div className="mt-4 flex justify-end">
                        {document.vendor && document.vendor._id && (
                          <Button
                            as={Link}
                            to={`/compliance-verification/${document.vendor._id}`}
                            size="sm"
                            variant="outline"
                          >
